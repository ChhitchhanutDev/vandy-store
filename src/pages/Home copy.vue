...
<template>
    <MainLayout>
        <section
            class="relative bg-gradient-to-br from-primary via-primary-dark to-[#5A3E36] text-white overflow-hidden">
                ...
                <div class="max-w-2xl">
                    ...
                    <RouterLink to="/products">
                        <ui-button variant="outline" size="lg"
                            class="inline-flex items-center justify-center px-6 py-3 border border-white rounded-lg text-white hover:bg-white hover:text-primary transition">View
                            Collection</ui-button>
                    </RouterLink>
                </div>
            </div>
            <div class="absolute right-0 top-0 w-1/3 h-full hidden lg:block">
                <svg viewBox="0 0 200 200" class="h-full w-full opacity-10" fill="currentColor">
                    <circle cx="100" cy="100" r="80" />
                    <circle cx="100" cy="100" r="50" />
                    <circle cx="100" cy="100" r="20" />
                </svg>
                <img src="/logo.png" alt="Vandy's Store"
                    class="absolute inset-0 m-auto h-80 w-80 rounded-full object-cover opacity-100" />
            </div>
        </section>
        ...
        <section class="bg-white border-y border-border py-16">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div v-if="product.categories.length" class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                    <RouterLink v-for="cat in product.categories.slice(0, 4)" :key="cat.id" to="/products"
                        class="group block p-6 rounded-[20px] bg-secondary/40 hover:bg-secondary transition-all duration-200 text-center">
                        <div
                            class="h-12 w-12 mx-auto mb-3 rounded-full bg-white flex items-center justify-center group-hover:scale-110 transition-transform">
                            <img v-if="cat.image_url" :src="cat.image_url" :alt="cat.name"
                                class="h-8 w-8 rounded-full object-cover" />
                            <svg v-else class="h-6 w-6 text-primary" fill="none" stroke="currentColor"
                                viewBox="0 0 24 24">
